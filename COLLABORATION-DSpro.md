...
| 案例 | 文档位置 | 教训 |
|------|---------|------|
| ASR 格式契约不一致（A 批 F1） | 已执行，见对话记录 | 改一端必须查另一端的 schema |
| handleLike 双重否定（A 批 F2） | 已执行，见对话记录 | 写 `!` 前推演 toggle 后状态 |
| 无限轮询（B 批 S1） | 已执行 | 异步流程必须列出所有终态 |
| 定时器泄漏（B 批 → C 批） | supplement-2 第〇节 | 复制粘贴是抽象信号，用循环 |

---

## 十一、产出物署名规范（执行者强制）

所有执行者产出的文件（自测报告、执行报告、修复方案、代码文件等），必须在以下三处标注署名 **DSpro**：

### 11.1 文件名
所有自测报告、执行报告、修复方案 MD 文件，以 `-DSpro` 结尾。

```
✅ 正确：selftest-2026-06-28-real-user-journey-DSpro.md
...
- 署名为最终责任标注。文件内容由 DSpro 负责，规划者审查后承担审核责任。
