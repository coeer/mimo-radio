...
### 2.4 当前测试基线（2026-07-22，最新）
- **后端**：305 测试全过（32 个 test 文件），tsc 零错误
- **前端**：204 测试全过（24 个 test 文件），tsc 零错误

> 基线演进：253/127（2026-06-29）→ 277/179（2026-07-18 技术债清理前）→ 288/189（P0a~P2）→ **305/204**（SSRF + F4 仲裁层 + backlog）。回退红线：不得低于 305/204。
...
│   ├── selftest-*.md                # 自测方法论 + 报告规范（常驻）
│   ├── archive/                     # 历史自测报告/评估（已归档）
│   └── plans/                       # 任务规格文档（执行者的输入，执行完移入 archive）
├── logs/
...
7. **DJ 串词字数**：intro / transition / chat 三入口统一 **60-120 字**（2026-06-29 统一，原 50-100/80-150/30-80 已废弃）。对齐视频深度。
...
**每个任务完成后，执行者必须写一份报告 MD 文件并落盘到自己的工作空间。** 这不是可选步骤——没有报告 = 任务未完成，规划者打回。

**为什么强制**：规划者不在执行者的会话里，看不到执行者的操作过程。报告是规划者理解"干了什么、改了哪些文件、测试结果、有没有问题"的唯一途径。**没有报告，规划者无法审查。**

#### 报告文件位置与命名
- 放在执行者自己的工作空间 `reports/` 子目录
- 命名：`exec-<主题>-<YYYY-MM-DD>-<执行者名>.md`（执行报告）或 `selftest-<主题>-<YYYY-MM-DD>-<执行者名>.md`（自测报告）
- 三处署名（文件名后缀 + 头部 author + 尾部落款），见第十一节
...
## 一、执行摘要
（一段话：做了什么、改了几个文件、结果如何。3-5 行，让规划者 30 秒内理解全貌）

## 二、改动明细
（表格：文件 | 改动内容 | 行号。让规划者能逐文件核实）

## 三、验证结果
- tsc：零错误 / 列出错误
- vitest：后端 N passed / 前端 N passed
- E2E（如有）：场景 + 结果 + 证据（日志计数 / DOM 检查值 / Profiler 数据）

## 四、与规格的偏差（如有）
（执行者如果偏离了规格——改了规格外的东西、用了不同方案——必须在此声明。
没有偏差就写"无偏差"。隐瞒偏差 = 违反契约。）

## 五、自评
（执行者自己发现的问题、风险、改进建议。没有就写"无"。）

## 六、前科复盘（如适用）
（如果本次任务涉及历史教训——接口约束、闭包 bug 等——在此说明如何遵守了。新执行者首次任务可跳过。）
...
| 维度 | 合格 | 不合格 |
|------|------|--------|
| 改动明细 | 逐文件 + 行号 + 改动描述 | "改了几个文件"无细节 |
| 验证结果 | 具体数字（305 passed）+ E2E 证据 | "测试通过"无数字 |
| 偏差声明 | 主动说明（即使"无偏差"） | 隐瞒或遗漏 |
| E2E（性能类任务） | 附 Profiler / DOM 检查数据 | 写"待实测"（铁律 5） |
| 诚实度 | 如实报告失败和问题 | 美化、隐瞒、"0 bug"万能句 |
...
> **项目已接入 GitHub**：`https://github.com/coeer/mimo-radio`。
> **策略**：Trunk-based（单分支）——这是单人 + AI 线性开发项目，不需要分支隔离。

### 三条规则（只有这三条）

1. **每次改动验证通过后，直接在 master 上 `git add + commit + push`**
   ```bash
   git add -A && git commit -m "feat: 简述改动" && git push origin master
   ```
   - 自动备份，无需规划者提醒。
   - 一个逻辑变更一条 commit，不要每改一行就 commit。

2. **Commit message 用 Conventional Commits 前缀**
   ```
...
3. **push 前必须 tsc 零错误 + vitest 全过**
   - master 的测试基线：后端 ≥305、前端 ≥204。
   - 保证 master 永远可运行。

### 为什么不用分支（GitFlow）

本项目是**单人 + AI 线性开发**：一个任务做完才做下一个，没有并行开发，没有 CI/CD，没有多人 Code Review。分支策略的隔离价值在此场景为零，反而增加认知负担（AI 执行者每次是新会话，分支操作易出错）。Trunk-based 满足"自动备份 + 可回滚"的核心需求，摩擦最小。

### 回滚（出问题时）

```bash
git log --oneline -5           # 看最近 5 条 commit
git revert <commit-hash>       # 撤销某次改动（生成反向 commit）
git push origin master         # 推送回滚
```
...
### ❌ 陷阱 4：git push 前不跑测试
项目已接入 git（trunk-based，直接 commit 到 master）。**push 前必须 tsc + vitest 全过**——否则 master 不可运行。详见第五-二节 Git 规范。
...
## 七、当前项目状态与待办（2026-06-29 更新）

### ✅ 已完成
- AI 推荐歌单（MiMo + 本地相似度算法 + 搜索前置精准点歌）
- AI DJ 语音播报（开场白 + 换歌串词 TTS，三引擎）
- **DJ 三层记忆闭环**：人格（persona）+ 短期（djMemory：已播歌曲/DJ串词/用户发言/时段）+ 长期（feedback 喂回搜索+品味注入）
- 聊天式推荐 + 换歌 + 推荐卡片（搜索前置：DJ 说的歌=实际播放的歌）
- 专辑封面（网易云 + QQ 双源）、真实歌词 LRC（双轨：有 LRC 逐句高亮，无则沉浸态）
- 每日电台时间轴页 `/plan`（终端风格 + 真实天气 + AI 时段 + 可点击播放）
- 个人主页 `/profile`（真实统计 + SVG 雷达图 + 数据不足降级）
- 反馈落库（feedback 表 + like/unlike/skip/complete + getLikedArtists 喂回推荐）
- ASR 语音输入、MediaSession、键盘控制、深浅主题（useTheme 统一）、粒子背景
- **体验流畅性**：换歌 isTransitioning 防重入 + "换台中..."反馈；KimiCard/FullscreenPlayer 重渲染优化（ProgressBar/LyricDisplay 抽离）
- Git 版本控制接入（`https://github.com/coeer/mimo-radio`）

### 📋 已知待办（未做，按优先级）
| 优先级 | 项 | 说明 |
|--------|-----|------|
| 🟡 中 | P2 审查问题（上线前批量做） | helmet CSP、颜色对比度 WCAG AA、next/dynamic 代码分割、独立 ErrorBoundary |
| 🟡 中 | F4 isPlaying 架构债 | 24+ 写点缺仲裁层，当前靠批处理兜底（详见 HANDOVER 第五节） |
| 🟡 中 | `useAudioPlayer.sideffects.test.ts` 5 个 tsc 错误 | Song 缺 emotionTags，历史问题，运行时测试通过 |
| 🟢 低 | ~~UPnP~~ / 歌单导入端到端测试 | UPnP 已下线（2026-07-18，P1-3）；歌单导入依赖外部环境未验证 |
| ❌ 已否决 | SSE 流式 / WebSocket | grill-me 审议否决：解决的不是真实瓶颈（AI 速度才是），成本高收益模糊 |

### 📄 关键文档索引
| 文档 | 用途 |
|------|------|
| `HANDOVER.md` | 项目交接，含完整改动清单 + 决策记录 + 技术债 |
| `COLLABORATION.md` | **本文件**，协同契约 |
| `docs/claudio-rebuild-plan-v2.2.md` | 视频逐帧规格（对标基准，55 帧拆解） |
| `docs/selftest-spec.md` | 自测报告规范（命名/位置/内容结构） |
| `docs/selftest-comprehensive.md` | 自测方法论（三层验证 + 工具链） |
| `docs/selftest-real-user-journey.md` | 真实用户场景框架（8 场景） |
| `docs/selftest-pattern-scanning.md` | 缺陷模式扫描法（三类模式） |
| `docs/plans/` | 当前活动方案（执行完移入 `docs/archive/`） |
...
**铁律 6：删除功能时必须 grep 全项目（含 .md 文档），不只是代码文件。**
删功能只删 .ts/.tsx 是执行者的常见盲区。功能名/关键词散落在 HANDOVER 改动清单、COLLABORATION 待办、docs 自测报告里——删了代码但文档还写着"已实现"，会误导下一个接手的人。**删完后必须 `grep -rn "功能名" --include="*.md"` 确认文档零残留**（或标注"已删除"）。代码零残留 ≠ 功能零残留。
...
| F4 isPlaying 升级为 P0 | Mavis 独立审计 | audit-2026-07-03-independent §3 | "暂缓"的技术债触发条件变化时要重新评估（MediaSession+ASR 上线 → 竞态概率上升）；审计的价值在"数写点 + 分析触发场景"而非"觉得有问题" |
| JSON 兜底 mood=userInput | Mavis 独立审计 | 同上 §4.1 | catch 兜底返回值不能用用户原始输入（会喂给下游匹配逻辑造成意外命中）；兜底要用中性值 |
| String(err) 13 处漏改 | Mavis 独立审计 | 同上 §4.2 | 已有 toErrorMeta 但未全量复用 = 历史重构不彻底；新工具引入后要 grep 全仓确认无残留旧写法 |
| chat 无取消 + 连发丢回复 | Mavis 独立审计 | 同上 §4.4 | 异步请求（fetch）+ last-write-wins 更新 = 连发时前几条回复静默丢失；和 F4 isPlaying 同源（多入口无仲裁） |
| 删 MediaSession 只删代码不删文档 | MiNiMax 删除任务 | 2026-07-05 remove-media-session | 删功能只 grep .ts/.tsx 不 grep .md = 文档残留"已实现"误导后人；**代码零残留 ≠ 功能零残留**（铁律 6） |
| body-parser 挂载顺序反了 | ZCode 指示错误 / KIMI 实验证伪 | 2026-07-18 verdict-p0b-1-body-parser-order | 规划者凭印象写"路径级在全局之后"，被 KIMI 用一次性实验推翻——body-parser 按注册顺序执行，先解析成功设 `_body=true` 让后续跳过，超限抛 413 到不了下一个；**路径级放宽必须在全局之前**。教训：规划者给中间件/配置顺序类指示必须自己先验证，不能凭记忆；双规划者架构的价值在事实层面互相核实（权威来自源码+实验，不来自身份）|
...
每个执行者在 `docs/` 下有自己的工作空间目录，内含一份 `AGENT.md` 身份卡：

```
docs/
├── DSpro/          ← DSpro 工作空间（AGENT.md + audits/reports/sessions/...）
├── DSflash/        ← DSflash 工作空间
└── MiNiMax/        ← MiNiMax 工作空间
```

**AGENT.md 是执行者的第一份必读文件**——它只含署名 + 指向本主文件的指针，不含规则内容。执行者进场后：先读 AGENT.md 确认"我是谁"，再读本主文件学规则。

### 11.2 署名三要素

所有执行者产出的文件（自测报告、执行报告、修复方案等），必须在以下三处标注**自己的署名**：

**① 文件名**：以 `-<执行者名>` 结尾
```
✅ exec-2026-06-29-fluency-DSpro.md
✅ selftest-2026-06-28-journey-DSflash.md
❌ exec-2026-06-29-fluency.md（无署名）
```
...
- 署名为最终责任标注。文件内容由署名执行者负责，规划者审查后承担审核责任。
- AGENT.md 永不需要同步更新（它只有署名 + 指针，规则变更只改本主文件）。
